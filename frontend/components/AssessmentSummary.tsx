...
const FRPSS_THEMES = [
  { label: "갈등·공격적 상호작용", items: ["FRPSS_01", "FRPSS_03", "FRPSS_08", "FRPSS_15", "FRPSS_16", "FRPSS_17"], prompt: "갈등이 시작되는 말과 행동, 강압이나 비꼼이 반복되는 장면을 확인" },
  { label: "회피·대화단절", items: ["FRPSS_02", "FRPSS_05", "FRPSS_07", "FRPSS_09", "FRPSS_18"], prompt: "대화가 중단되는 상황과 먼저 대화를 피하는 가족원을 확인" },
  { label: "정서적 거리·친밀감 저하", items: ["FRPSS_04", "FRPSS_06", "FRPSS_10", "FRPSS_11", "FRPSS_14"], prompt: "함께하는 시간과 애정 표현이 줄어든 시점과 예외 장면을 확인" },
  { label: "가족 내 편안함·상호관심", items: ["FRPSS_12", "FRPSS_13"], prompt: "집에서 느끼는 긴장과 서로의 어려움을 나누기 힘든 이유를 확인" },
] as const;
...
export default function AssessmentSummary({ assessments, responses = [], compact = false }: AssessmentSummaryProps) {
  ...
  const frpss = byCode.get("FRPSS");
  const stress = byCode.get("FSTRESS");
  const divorce = byCode.get("DIVORCE");
  const relationship = relationshipSummary(frpss, responses);
...
function relationshipSummary(frpss: AssessmentScore | undefined, responses: QuestionnaireResponse[]) {
  const responseById = new Map(responses.filter(item => item.section === "FRPSS").map(item => [item.item_id, item.response_value]));
  const rankedThemes = FRPSS_THEMES.flatMap(theme => {
  ...
  if (!frpss) {
    return {
      ...
      evidence: "가족관계문제징후척도 응답 없음",
    };
  }
  const criterion = frpss.score >= 54 ? "확인 기준(54점) 이상" : "확인 기준(54점) 미만";
  const themeText = leading ? `\n상담 준비용 문항 묶음 중 ‘${leading.label}’ 응답이 상대적으로 높게 나타남` : "";
  return {
    summary: `총 ${formatScore(frpss.score)}/90점 · ${criterion}${themeText}`,
    prompt: leading?.prompt ?? "높게 응답한 문항의 실제 장면과 반복 빈도를 확인",
    evidence: `총점 ${formatScore(frpss.score)}/90 · 확인 기준 54`,
  };
}

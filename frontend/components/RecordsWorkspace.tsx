...
const sessionContentFieldMaxLength = 700;
...
export default function RecordsWorkspace({ activeStep, onStepChange, clientId, sessionNumber, sessionDate, hasNextSession, goal, note, sourceLabel, onFinalized }: { activeStep: number; onStepChange: (step: number) => void; clientId: string; sessionNumber: number; sessionDate: string; hasNextSession: boolean; goal: string; note: string; sourceLabel: string; onFinalized?: (nextSessionNumber: number | null) => void }) {
  ...
  async function extractOCR() {
    ...
    try {
      ...
      setError("SOAP 노트 내용을 불러오지 못했습니다. 잠시 후 다시 시도하거나 직접 입력해 주세요.");
  ...
  async function finalizeRecord() {
    if (!records) return;
    const soapText = includeSoap ? soapFields.filter(field => soapDraft[field].trim()).map(field => `${field}: ${soapDraft[field].trim()}`).join("\n") : "";
    if (files.length && ocrResult && !ocrReviewed) return setError("SOAP 노트 내용이 변경되었습니다. 원본 확인을 다시 완료하고 기록 초안을 재생성하세요.");
    if (generatedOCRSource !== soapText) return setError("SOAP 입력이 초안 생성 후 변경되었습니다. 기록 초안을 다시 생성하세요.");
  ...
  const ocrRecordIsStale = Boolean(records && generatedOCRSource !== soapSourceText);
  const ocrReady = Boolean(ocrResult && ocrText.trim() && ocrReviewed);
  const hasInputSource = Boolean(sessionText.trim() || soapSourceText.trim());
  ...
  return <section className="records-workspace">
    {activeStep === 3 && <Panel className="workflow-deck-card counseling-source-stage">
      ...
      <div className="panel-heading"><div><h2>이번 상담 내용을 입력하세요</h2><span>상담 메모를 직접 입력하거나 작성한 SOAP 노트를 첨부하면 다음 단계로 진행할 수 있습니다.</span></div><Tag>{sourceLabel}</Tag></div>
      <label className="session-source-input">상담 메모 직접 입력<textarea rows={7} value={sessionText} onChange={event => setSessionText(event.target.value)} placeholder="실제 상담에서 확인한 내담자 보고, 상담사 관찰, 개입과 반응을 입력하세요."/></label>
      <label className={`optional-soap-toggle ${includeSoap ? "selected" : ""}`}><input type="checkbox" checked={includeSoap} onChange={event => { setIncludeSoap(event.target.checked); if (!event.target.checked) selectFiles([]); setError(""); }}/><span><b>SOAP 노트 추가</b><small>선택 사항 · 직접 입력하거나 작성한 노트를 첨부할 수 있습니다.</small></span></label>

      {includeSoap && <div className="optional-soap-workspace">
        <div className="soap-upload-toolbar">
          <div><b>SOAP 노트 첨부</b><span>{files.length ? `${files.length}개 파일 선택됨 · ${files.map(file => file.name).join(", ")}` : "선택한 파일이 없습니다. 직접 입력만 사용해도 됩니다."}</span></div>
        ...
        {files.length === 0
          ? <div className="soap-direct-entry"><div className="soap-entry-heading"><b>SOAP 노트 직접 입력</b><span>파일을 첨부하지 않으면 입력 내용만 상담기록지 생성에 반영됩니다.</span></div><SoapFieldsEditor values={soapDraft} onChange={updateSoapField}/></div>
          : <>
            {ocrStatus && !ocrStatus.available && <div className="service-connection-note">SOAP 노트 내용을 불러올 수 없습니다. 오른쪽 입력란에 직접 작성할 수 있습니다.</div>}
            <div className="soap-review-layout">
              <section className="soap-original-column"><div className="soap-column-heading"><b>첨부한 SOAP 노트</b><span>선택한 파일</span></div><DocumentPreviews filePreviews={filePreviews}/><button className="primary wide" type="button" onClick={extractOCR} disabled={ocrBusy || !ocrStatus?.available}>{ocrBusy ? "SOAP 노트 불러오는 중…" : ocrResult ? "SOAP 노트 다시 불러오기" : "SOAP 노트 내용 불러오기"}</button></section>
              <section className="soap-structured-column"><div className="soap-column-heading"><b>SOAP 노트 내용</b><span>{ocrResult ? "원본과 비교해 필요한 내용을 수정하세요." : "내용을 불러오기 전에도 직접 입력할 수 있습니다."}</span></div><SoapFieldsEditor values={soapDraft} onChange={updateSoapField}/>
                {ocrResult && <><div className={`soap-safety-check ${ocrResult.risk_review_required ? "caution" : ""}`}><b>안전 관련 문구 확인</b><span>{ocrResult.risk_review_required ? "안전·위험 관련 문장은 부정 표현까지 포함해 원본과 비교해 주세요." : "불러온 안전 관련 문구를 원본과 대조해 주세요."}</span></div><label className={`ocr-review-confirm ${ocrReviewed ? "confirmed" : ""}`}><input type="checkbox" checked={ocrReviewed} onChange={event => setOCRReviewed(event.target.checked)}/><span><b>원본 확인 완료</b>첨부한 SOAP 노트와 S/O/A/P 내용을 확인하고 필요한 부분을 수정했습니다.</span></label></>}
              </section>
            </div>
          </>}
      </div>}
      {error && <p className="form-error">{error}</p>}
      <StageNavigation step={3} onStepChange={onStepChange} nextDisabled={!hasInputSource || Boolean(includeSoap && files.length && !ocrReady)} nextLabel="다음: 기록 작성"/>
    </Panel>}

    {activeStep === 4 && <>
      <Panel className="workflow-deck-card">
        <div className="workflow-card-kicker">4단계 · 기록 작성</div>
        <div className="panel-heading"><div><h2>{sessionNumber === 1 ? "초기상담기록지 초안" : `${sessionNumber}회기 상담기록지 초안`}</h2><span>확인한 SOAP 노트와 실제 상담 메모를 합쳐 편집 가능한 공식 기록을 만듭니다.</span></div><Tag>{sourceLabel}</Tag></div>
        <div className="record-workflow-guide"><b>{sessionNumber === 1 ? "필수 · 초기상담기록지" : "필수 · 상담기록지"}</b><span>SOAP 노트는 선택사항이며, 필수 기록은 상담사가 확인한 뒤 확정합니다.</span></div>
        <div className="record-source-summary"><div><span>상담 메모</span><b>{sessionText.trim() ? "입력 완료" : "사용하지 않음"}</b></div><div><span>SOAP 노트</span><b>{includeSoap ? files.length ? "첨부·확인 반영" : "직접 입력 반영" : "사용하지 않음"}</b></div></div>
    ...
    {records && <Panel>
      ...
      <div className="record-tabs">
        ...
        {includeSoap && <button className={recordTab === "soap" ? "active" : ""} onClick={() => setRecordTab("soap")}>SOAP 노트</button>}
      ...
      <div className="session-finalize-box">
        ...
        <p>{includeSoap ? "필수 기록과 SOAP 노트를 함께 보관합니다." : "필수 기록만 보관합니다."} 확정된 기록은 다음 회기 준비자료에 반영됩니다.</p>
        {ocrRecordIsStale && <p className="form-error">SOAP 노트 내용 또는 보완 메모가 바뀌었습니다. 변경 내용을 반영하도록 기록 초안을 다시 생성하세요.</p>}
      ...
      <div className="download-row"><button onClick={() => downloadFile("integrated_records.txt", recordsText, "text/plain;charset=utf-8")}>통합 기록 문서</button><button onClick={() => downloadFile("integrated_records.json", JSON.stringify(records, null, 2), "application/json")}>통합 기록 백업</button><button onClick={() => downloadFile("soap_note_text.txt", ocrText, "text/plain;charset=utf-8")} disabled={!ocrText}>SOAP 노트 정리문</button></div>
    </Panel>}
...
function SessionRecordForm({ sessionNumber, values, onChange }: { sessionNumber: number; values: Record<string, string>; onChange: (field: string, value: string) => void }) {
  return <article className="official-record-form session-record-form">
    ...
    <div className="record-table-wrap"><table><tbody>
      ...
      <tr><th rowSpan={3}>상담 내용<br/><small>(상담개입)</small></th><td colSpan={3}><RecordTextarea value={values[sessionFields[4]]} onChange={value => onChange(sessionFields[4], value)} rows={10} maxLength={sessionContentFieldMaxLength}/></td></tr>
...
function RecordTextarea({ value = "", onChange, rows, maxLength = officialRecordFieldMaxLength }: { value?: string; onChange: (value: string) => void; rows: number; maxLength?: number }) {
  return <>
    <textarea className="record-screen-textarea" value={value} onChange={event => onChange(event.target.value)} rows={rows} maxLength={maxLength}/>

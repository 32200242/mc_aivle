  ...
  return <AppShell title="안녕하세요, 윤주연 상담사님" subtitle="오늘도 따뜻한 상담으로 내담자의 변화를 함께 만들어가요.">
    <div className="counselor-home-grid">
      ...
      <Panel className="schedule-panel">
        ...
        {loading ? <p className="schedule-empty">상담 일정을 불러오는 중입니다.</p> : selectedAppointments.length === 0 ? <p className="schedule-empty">선택한 날짜에 예정된 상담이 없습니다.</p> : <div className="schedule-list">{selectedAppointments.map(({ client, date }) => {
          const status = client.id === HWANG_DEMO_CLIENT_ID
            ? { label: "상담기록지 작성 필요", tone: "pink" }
            : appointmentStatus(date, now);
          return <Link className="appointment" href={`/counselor/clients/${client.id}`} key={`${client.id}-${date.toISOString()}`} aria-label={`${client.name} 상담 일정 상세 보기`}>
...
function appointmentStatus(appointment: Date, now: Date | null): { label: string; tone: string } {
  if (!now || now.getTime() < appointment.getTime()) return { label: "예정", tone: "gray" };
  const endTime = appointment.getTime() + 50 * 60 * 1000;
  if (now.getTime() < endTime) return { label: "진행 중", tone: "green" };
  return { label: "상담기록지 작성 필요", tone: "pink" };
}

...
import { HWANG_DEMO_CLIENT_ID } from "@/data/hwangCopilotDemo";
  ...
  return <AppShell title="내담자 관리" subtitle="접수정보·사전문진·계산 점수·회기 기록을 한 사례로 연결해 확인합니다.">
    ...
    {activeTab === "records" && <><Panel><div className="panel-heading"><div><h2>전체 회기 흐름</h2><span>완료 기록과 예정 회기를 시간 순서로 확인합니다.</span></div></div><div className="journey">{client?.sessions.map((session, index) => <span className="journey-item" key={session.id}><div><b>{session.number}회기</b><span>{session.official_record?.fields["상담내용"] || session.official_record?.fields["상담내용(상담개입)"] || session.client_report || session.change_since_last || "진행 예정"}</span></div>{index < (client?.sessions.length ?? 0) - 1 && <i>→</i>}</span>)}</div></Panel><Panel><div className="panel-heading"><div><h2>{current ? `${current.number}회기 확정 기록` : "확정된 회기 기록"}</h2><span>{current?.official_record?.record_label ?? (current ? "저장된 회기 기록" : "첫 회기 완료 후 표시됩니다.")}</span></div>{current && <Tag>{current.date}</Tag>}</div>
      {!current ? <div className="empty-analysis"><b>아직 확정된 상담 기록이 없습니다.</b><p>첫 상담을 완료하고 기록을 확정하면 이곳에 표시됩니다.</p></div> : current.official_record ? <>
        ...
        {client?.id === HWANG_DEMO_CLIENT_ID && current.number === 1 && <>
          <dl className="client-appointment-facts session-record-facts"><div><dt>상담 대상</dt><dd>{current.participants.join(", ")}</dd></div><div><dt>상담 방법</dt><dd>{current.modality}</dd></div><div><dt>회기 목표</dt><dd>{current.goal}</dd></div></dl>
          <div className="official-record-summary session-record-notes"><div><b>주요 개입</b><p>{current.interventions.join(" · ")}</p></div><div><b>과제·다음 회기</b><p>{current.homework}<br/>{current.next_plan}</p></div></div>
        </>}

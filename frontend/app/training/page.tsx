...
const WARNING_FEEDBACK_FALLBACK = "상대방의 의도나 내담자의 감정을 성급히 단정하지 않습니다.";
const WARNING_FEEDBACK_ALIASES = ["주의할응답", "주의할 응답", "주의사항", "피해야 할 응답"] as const;


function feedbackValueText(value: TurnResult["supervisor_feedback"][string] | undefined) {
...
function normalizedFeedbackEntries(feedback: TurnResult["supervisor_feedback"]) {
  const consumed = new Set<string>();
  const sections: { key: string; label: string; text: string }[] = [];
  const coachingSections = [
    { key: "강점", label: "강점", aliases: ["강점"] },
    { key: "보완점", label: "보완점", aliases: ["보완점"] },
    { key: "권장질문", label: "권장 질문", aliases: ["권장질문", "권장 질문"] },
    { key: "주의할응답", label: "주의할 응답", aliases: [...WARNING_FEEDBACK_ALIASES] },
  ];
  ...
  for (const section of coachingSections) {
    ...
    for (const alias of section.aliases) {
      consumed.add(alias);
      if (!text) text = feedbackValueText(feedback[alias]);
    }
    if (section.key === "주의할응답" && !text) text = WARNING_FEEDBACK_FALLBACK;
    if (text) sections.push({ key: section.key, label: section.label, text });
  }

  for (const [key, value] of Object.entries(feedback)) {
    if (consumed.has(key)) continue;
    const text = feedbackValueText(value);
    if (text) sections.push({ key, label: key, text });
  }
  return sections;
}

...
AUDIO_GUIDANCE_SCALE = os.getenv("LONGCAT_AUDIO_GUIDANCE_SCALE", "0.78")
...
def _prompt(emotion: str) -> str:
    ...
    return " ".join(
        [
            ...
            emotional_detail,
            "Speech uses accurate but compact lip shapes with restrained small-scale articulation.",
            "Her lips separate only as much as necessary; jaw opening is shallow and chin motion is limited.",
            "Lower cheeks and nasolabial folds move only subtly, without pumping on stressed syllables.",
            "Teeth appear only briefly when naturally required, and the mouth fully rests during silence.",
            ...
            "No wide mouth shapes, exaggerated vowel rounding, lip stretching, grimacing or oversized articulation.",
...
def _assert_runtime_ready() -> None:
    ...
    ).replace("offload_kv_cache=False", "offload_kv_cache=True").replace(
        "audio_guidance_scale = 1.0",
        "audio_guidance_scale = float(os.environ.get('LONGCAT_AUDIO_GUIDANCE_SCALE', '0.78'))",
    )
    if patched != script:
...
def render(
    ...
    environment["LONGCAT_AUDIO_GUIDANCE_SCALE"] = AUDIO_GUIDANCE_SCALE

...
FRPSS_CONFIRMATION_CUTOFF = 54


FRPSS_ITEMS = [
    ("FRPSS_01", "hostility", "서로 간에 불평, 불만이 많다"),
    ("FRPSS_02", "avoidance", "갈등이 생기면 해결하려 하기보다 피해버린다"),
    ("FRPSS_03", "hostility", "가족원 간에 욕설이나 큰소리를 내며 싸운다"),
    ("FRPSS_04", "distance", "가족이 함께 사진 찍고 싶어 하지 않는다"),
    ("FRPSS_05", "distance_avoidance", "집에 같이 있어도 서로 얼굴을 마주치고 싶어 하지 않는다"),
    ("FRPSS_06", "distance", "함께 식사·여행·외출·쇼핑 등을 하고 싶어 하지 않는다"),
    ("FRPSS_07", "communication", "가족 안에서 자신의 힘든 일을 말하는 것이 어렵다"),
    ("FRPSS_08", "conflict_proneness", "가족원 간에 의견충돌이 쉽게 일어난다"),
    ("FRPSS_09", "communication_breakdown", "가족 안에 대화다운 대화가 없다"),
    ("FRPSS_10", "distance_intimacy", "포옹·손잡기 등의 친밀한 표현이 거의 없다"),
    ("FRPSS_11", "affective_flatness", "가족 안에 웃음이 거의 없다"),
    ("FRPSS_12", "home_comfort", "집에 있으면 마음이 편하지 않고 불편하다"),
    ("FRPSS_13", "emotional_sharing_deficit", "서로의 고민과 상황에 관심이 없고 잘 알지 못한다"),
    ("FRPSS_14", "distance_intimacy", "사랑한다는 표현을 직접적이든 간접적이든 하지 않는다"),
    ("FRPSS_15", "hostility", "서로 짜증스럽게 말한다"),
    ("FRPSS_16", "hostility_contempt", "서로 가시가 있는 말·비꼬는 말·공격적인 말을 한다"),
    ("FRPSS_17", "control_coercion", "한 명이 일방적·강압적으로 자기 의사를 강요한다"),
    ("FRPSS_18", "communication_breakdown_avoidance", "꼭 필요한 말이 아니면 서로 이야기하지 않는다"),
]
...
def questionnaire_items() -> list[dict[str, Any]]:
    ...
    order = 0
    for item_id, domain, text in FRPSS_ITEMS:
        order += 1
        rows.append(_item(item_id, "FRPSS", domain, text, "likert_1_5", 1, 5, False, "family_crisis_scale_2013", order))
...
def calculate_assessments(responses: dict[str, int]) -> list[dict[str, Any]]:
    responses = {
        canonical_questionnaire_item_id(item_id): value
        for item_id, value in responses.items()
    }
    items = {item["item_id"]: item for item in questionnaire_items()}
    frpss_values = [responses[item_id] for item_id in responses if item_id.startswith("FRPSS_")]
    ...
    frpss = sum(max(1, min(5, int(value))) for value in frpss_values)
    ...
    frpss_severity = "확인 기준 이상" if frpss >= FRPSS_CONFIRMATION_CUTOFF else "확인 기준 미만"
    assessments = [
        _assessment(
            "FRPSS", "가족관계문제징후척도", frpss, 90, frpss_severity,
            f"18문항 원점수 합계이며 확인 기준은 {FRPSS_CONFIRMATION_CUTOFF}점(문항 평균 3.0점) 이상임",
        ),
        _assessment(
...
def canonical_questionnaire_item_id(item_id: str) -> str:
    """Map legacy FRPS item IDs to the official FRPSS acronym."""

    return f"FRPSS_{item_id.removeprefix('FRPS_')}" if item_id.startswith("FRPS_") else item_id


def canonical_questionnaire_section(section: str) -> str:
    """Map the legacy FRPS section label without changing stored demo data."""

    return "FRPSS" if section == "FRPS" else section

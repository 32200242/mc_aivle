...
class RecordModelOutputError(RuntimeError):
    """Raised when a model-only validation request cannot produce the record contract."""
...
def _official_length(field: str, value: Any) -> str:
    text = str(value or "").strip()
    maximum = official_record_field_max_length(field)
    if len(text) <= maximum:
        return text
...
def _missing_record_fields(data: dict[str, Any], request: RecordGenerateRequest) -> list[str]:
    ...
    for primary_key, alternate_key, fields in groups:
        ...
        if not isinstance(raw, dict):
            missing.add(primary_key)
            continue
        missing.update(f"{primary_key}.{field}" for field in fields if field not in raw)
        if primary_key == "soap":
            missing.update(
...
def _fallback_records(
    ...
    *,
    ...
    initial = {
        field: _official_length(field, value) if field in INITIAL_FIELDS else value
        for field, value in initial.items()
    }
    session = {
        field: _official_length(field, value) if field in SESSION_FIELDS else value
    ...
    if request.ocr_text:
        uncertain.append("첨부한 SOAP 노트와 정리된 S/O/A/P 내용이 일치하는지 확인해 주세요.")
        uncertain.extend(f"SOAP 노트 확인 항목: {flag}" for flag in request.ocr_review_flags)
    return IntegratedRecords(
        ...
        source_summary={
            "상담 메모": "반영됨" if request.transcript else "입력 없음",
            "SOAP 노트": "반영됨" if request.ocr_text else "첨부 없음",
            "원본 확인": "완료" if request.ocr_text and request.ocr_reviewed else "해당 없음",
            "확인 메모": request.ocr_review_note or "입력 없음",
            "상담사 보완 메모": "반영됨" if notes else "입력 없음",
            "기존 상담 요약": "반영됨" if request.existing_summary else "입력 없음",
            "사례 기본정보": "반영됨" if case is not None else "연결 없음",
            "현재 회기 기록": "반영됨" if stored_session_source else "입력 없음",
        },
    )
...
def _record_messages(
    ...
    *,
    ...
    target_template = {field: "" for field in target_fields}
    if request.record_type == "initial_intake":
        # The frontend renders the genogram as SVG from this evidence text.  Keep
        # the key present even when the source contains no confirmed family data.
        target_template["가계도"] = "확인 필요"
    output_template: dict[str, Any] = {target_key: target_template, "uncertain_items": []}
    if request.include_soap:
        output_template["soap"] = {field: "" for field in SOAP_FIELDS}
    soap_rule = (
        "soap 키도 만들고 S, O, A, P를 각각 주관적 보고, 직접 관찰, 사례평가, 다음 계획으로 구분한다."
        if request.include_soap else "SOAP는 요청되지 않았으므로 생성하지 않는다."
    )
    length_rule = (
        '"상담내용(상담개입)"은 700자 이내로 충분히 작성하고, 나머지 각 값은 2문장 또는 300자 이내로 쓴다.'
        if request.record_type == "session_record"
        else "각 값은 한국어 문자열이며 2문장 또는 300자 이내로 쓴다."
    )
    system = f"""
...
2. 같은 항목이 충돌하면 상담사 수정보완 > 상담사 메모 > 상담 대화 > 확정요약 > 첨부 SOAP 노트 순서로 쓴다.
3. 자료에 없는 인적사항·사건·발언·진단·위험·개입·효과·계획을 만들지 않는다. 없으면 "확인 필요", 선택 항목이면 ""로 둔다.
4. 내담자 보고, 상담사 직접 관찰, 실제 개입, 확인된 반응을 섞지 않는다.
5. 다음 계획은 입력에 명시된 합의나 계획만 쓴다.
6. 가계도는 SVG가 아니라 렌더링 근거가 되는 가족관계 텍스트로 쓴다. 확인된 가족구성·동거·관계만 사용한다.
   일부만 확인되면 확인된 내용 뒤에 "확인 필요"를 표시하고, 아무 근거도 없으면 값은 정확히 "확인 필요"로 쓴다.
   초기상담기록지에서는 가계도 키를 절대 생략하지 않는다.
7. 초기상담기록지의 상담계획에는 근거에서 확인된 다음 회기 계획·과제·목표 실행 절차를 빠뜨리지 않는다.
...
아래 JSON 틀의 키를 삭제하거나 이름을 바꾸지 말고 값만 작성한다:
{json.dumps(output_template, ensure_ascii=False)}
각 값은 한국어 문자열로 작성한다. {length_rule}
...
""".strip()
    context = request.model_dump(exclude={
        "record_type", "include_soap", "require_model_output", "client_id", "session_number",
    })
...
async def generate_records(
    ...
    *,
    ...
    if settings.ai_provider == "mock":
        if request.require_model_output:
            raise RecordModelOutputError(
                "MODEL_OUTPUT_INVALID: 실모델 전용 요청에서는 mock 응답을 사용할 수 없습니다."
            )
    ...
    if not data or missing:
        missing_text = ", ".join(missing) if missing else "완결된 JSON 객체"
        retry_messages = [
            {
                "role": "system",
                "content": messages[0]["content"]
                + "\n재시도 규칙: 직전 출력은 계약을 충족하지 못했다. 각 필드를 한 문장으로 줄이고 "
                + f"다음 누락 항목을 모두 복구한다: {missing_text}. 완결된 JSON만 출력한다.",
            },
            messages[1],
            {"role": "assistant", "content": raw[:6000]},
            {"role": "user", "content": "위 초안을 출력 계약에 맞는 완결된 JSON 객체로 교정하라."},
        ]
        try:
            retry_raw = await chat_completion(retry_messages, max_tokens=1600, temperature=0.0)
    ...
    if not data:
        if request.require_model_output:
            raise RecordModelOutputError(
                "MODEL_OUTPUT_INVALID: 두 번의 생성 모두 구조화된 JSON 객체로 해석되지 않았습니다."
            )
    ...
    if missing:
        if request.require_model_output:
            raise RecordModelOutputError(
                "MODEL_OUTPUT_INVALID: 두 번의 생성 후에도 필수 기록 항목이 누락되었습니다: "
                + ", ".join(missing)
            )
    ...
    if normalized is fallback:
        if request.require_model_output:
            raise RecordModelOutputError(
                "MODEL_OUTPUT_INVALID: 모델 응답을 공식 기록 구조로 정규화할 수 없습니다."
            )
        return fallback.model_copy(update={

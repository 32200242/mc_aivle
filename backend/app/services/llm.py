...
def _local_midm_sync(messages: list[dict[str, str]], max_tokens: int, temperature: float) -> str:
    # Include the lazy model load in the same lock as generation.  functools'
    # cache may execute the wrapped function more than once when concurrent
    # first requests arrive before the first load has completed.
    with _LOCAL_LOCK:

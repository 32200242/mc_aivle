...
DEMO_FIRST_RESPONSES = {
    "couple-conflict-01": {
        "female": ("제가 남편이랑 얘기를 좀 해보려고 하면요, 남편은 제 말을 자기를 탓하는 걸로 받아들여요. "
...
WARNING_FEEDBACK_FALLBACK = "상대방의 의도나 내담자의 감정을 성급히 단정하지 않습니다."
WARNING_FEEDBACK_ALIASES = ("주의할응답", "주의할 응답", "주의사항", "피해야 할 응답")


def _feedback_sentences(value: Any) -> list[str]:
...
def _normalize_feedback(value: Any) -> dict[str, list[str] | str | int | float]:
    if not isinstance(value, dict):
        normalized: dict[str, list[str] | str | int | float] = {
            "보완점": ["슈퍼바이저 피드백 형식을 확인해 주세요."]
        }
    else:
        normalized = dict(value)

    warning_sentences: list[str] = []
    for alias in WARNING_FEEDBACK_ALIASES:
        alias_value = normalized.pop(alias, None)
        if not warning_sentences:
            warning_sentences = _feedback_sentences(alias_value)
    normalized["주의할응답"] = warning_sentences or [WARNING_FEEDBACK_FALLBACK]
    return normalized
...
def _normalize_result(data: dict[str, Any]) -> TurnResult:
    ...
    feedback = _normalize_feedback(
        _first_value(data, "supervisor_feedback", "슈퍼바이저 피드백", "피드백")
    )
    return TurnResult(
        turn_id=f"turn-{uuid.uuid4().hex[:12]}",
        response=response,

...
def _validate_official_record(
    ...
    *,
    ...
    too_long = [
        ...
        if len(str(record.get(field, "")).strip()) > official_record_field_max_length(field)
    ]
    if too_long:
        limits = ", ".join(f"{field}은 {official_record_field_max_length(field)}자 이내" for field in too_long)
        raise ValueError(
            f"공식 기록지 항목별 글자 수를 확인해 주세요: {limits}"
        )

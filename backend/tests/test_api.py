...
def test_demo_first_question_is_fixed_and_prewarmable() -> None:
    ...
    assert "제 마음은 하나도 전해지지 않은 것 같아요" in response_text
    assert '"emotion": "hurt"' in response_text
...
@pytest.mark.parametrize(
    ("feedback", "expected"),
    [
        ({"주의할응답": ["잘잘못을 단정하지 않습니다."]}, "잘잘못을 단정하지 않습니다."),
        ({"주의할 응답": "감정을 단정하지 않습니다."}, "감정을 단정하지 않습니다."),
        ({"주의할응답": []}, ai.WARNING_FEEDBACK_FALLBACK),
        ({}, ai.WARNING_FEEDBACK_FALLBACK),
        (None, ai.WARNING_FEEDBACK_FALLBACK),
    ],
)
def test_training_feedback_warning_is_normalized(feedback, expected: str) -> None:
    payload = {
        "response": "조금 더 이야기해 보고 싶어요.",
        "emotion": "neutral",
        "supervisor_feedback": feedback,
    }
    result = ai._normalize_result(payload)
    assert result.supervisor_feedback["주의할응답"] == [expected]
...
def test_counselor_id_login_and_case_assignment_are_linked() -> None:
    ...
    assert "FRPSS" in {item["code"] for item in detailed.json()["assessments"]}
    assert all(
        not item["item_id"].startswith("FRPS_")
        for item in detailed.json()["questionnaire_responses"]
    )
...
def test_session_record_content_uses_700_character_limit_only() -> None:
    ...
    record["상담내용(상담개입)"] = "가" * 701
    with pytest.raises(ValueError, match="700자 이내"):
        session_workflow._validate_official_record(
            record,
            session_number=4,
            service_date=session.date,
    ...
    record["상담내용(상담개입)"] = "확인된 상담 내용"
    record["다음 회기 계획"] = "가" * 301
    with pytest.raises(ValueError, match="300자 이내"):
        session_workflow._validate_official_record(
            record,
            session_number=4,
            service_date=session.date,
...
def test_record_fallback_narratives_respect_official_length() -> None:
    ...
    assert len(fallback.session_record["상담내용(상담개입)"]) <= 700
    assert all(
...
def test_official_family_questionnaire_scoring_is_exposed_without_invented_risk_levels() -> None:
    ...
    assert scores["FRPSS"]["score"] == 90
    assert scores["FRPSS"]["max_score"] == 90
    assert scores["FRPSS"]["severity"] == "확인 기준 이상"
    assert "54점" in scores["FRPSS"]["interpretation"]
...
def test_legacy_frps_response_ids_are_read_as_frpss() -> None:
    responses = {item["item_id"]: item["scale_max"] for item in questionnaire_items()}
    legacy_responses = {
        item_id.replace("FRPSS_", "FRPS_"): value
        for item_id, value in responses.items()
    }
    scores = {item["code"]: item for item in calculate_assessments(legacy_responses)}
...
def test_record_model_only_mode_rejects_fallback(monkeypatch) -> None:
    monkeypatch.setattr(records, "settings", SimpleNamespace(ai_provider="midm"))

    async def incomplete_completion(*args, **kwargs):
        return '{"session_record":{},"uncertain_items":[]}'
    ...
    with pytest.raises(records.RecordModelOutputError, match="MODEL_OUTPUT_INVALID"):
        asyncio.run(records.generate_records(request))


def test_record_model_only_mode_rejects_mock(monkeypatch) -> None:
    monkeypatch.setattr(records, "settings", SimpleNamespace(ai_provider="mock"))
    request = RecordGenerateRequest(
        record_type="session_record",
        require_model_output=True,
        transcript="실모델 전용 검증 요청입니다.",
    )

    with pytest.raises(records.RecordModelOutputError, match="mock 응답을 사용할 수 없습니다"):
        asyncio.run(records.generate_records(request))


def test_missing_record_fields_are_unique() -> None:
    request = RecordGenerateRequest(record_type="session_record")
    missing = records._missing_record_fields(
        {"session_record": {"검증용 비계약 필드": "상위 객체가 존재하도록 하는 값"}},
        request,
    )

    assert len(missing) == len(records.MODEL_SESSION_FIELDS)
    assert len(missing) == len(set(missing))


def test_initial_record_prompt_keeps_genogram_key_and_unknown_rule() -> None:
    request = RecordGenerateRequest(
        record_type="initial_intake",
        transcript="가족관계 정보 없이 초기 호소문제만 확인했습니다.",
    )

    messages = records._record_messages(request)
    system_prompt = messages[0]["content"]

    assert '"가계도": "확인 필요"' in system_prompt
    assert "가계도 키를 절대 생략하지 않는다" in system_prompt
    assert '아무 근거도 없으면 값은 정확히 "확인 필요"' in system_prompt
    assert "SVG가 아니라 렌더링 근거가 되는 가족관계 텍스트" in system_prompt

...
Windows 최초 설치와 Python/Node 확인은 프로젝트 루트 [`README.md`](../README.md)의 `Windows 최초 1회 설치`를 먼저 따릅니다. 기존 `.venv`가 현재 시스템과 다른 Python 경로를 가리키면 README의 가상환경 재생성 절차를 적용합니다.

현재 프로젝트 루트:

```text
C:\path\to\family_center_final
```
